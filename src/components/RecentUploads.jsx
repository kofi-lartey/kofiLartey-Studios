...
const RecentUploads = ({ refreshTrigger, selectedGallery }) => {
  ...
  if (error) {
    return (
      <div className="text-center py-20 border border-red-500/20 rounded-3xl bg-red-500/[0.01]">
        <FiAlertCircle className="text-red-500 text-5xl mx-auto mb-4" />
        <p className="text-red-400 text-sm">Failed to load images</p>
        <p className="text-gray-500 text-xs mt-2">{error}</p>
        <button
          onClick={fetchImages}
          className="mt-4 px-4 py-2 bg-indigo-600 rounded-lg text-xs font-bold uppercase tracking-wider hover:bg-indigo-500 transition-colors"
        >
          Try Again
        </button>
      </div>
  ...
  if (images.length === 0) {
    return (
      <div className="text-center py-20 border border-white/5 rounded-3xl bg-white/[0.01]">
        ...
        <p className="text-gray-600 text-xs mt-2">Drag & drop images to start uploading.</p>
      </div>
    );
  }
      ...
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            ...
            Status: {galleryInfo?.galleryStatus || 'Active'}
          </p>
        </div>
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity p-4 flex flex-col justify-between">
                <div className="flex justify-end">
                  <button
                    ...
                    className="p-1.5 bg-black/50 rounded-lg text-red-400 hover:text-red-300 transition-colors"
                  >
                    <FiTrash2 size={14} />
                  </button>
                </div>
